...
export default function VoiceQuiz({ onComplete }) {
  ...
  const startSpeechRecognition = () => {
    ...
    recognition.onresult = async (event) => {
      ...
      // GUARD: Reject empty transcripts or simple dead space breaths immediately
  ...
  useEffect(() => {
    const greeting = stepIndex === 0 
      ...
      : `${confirmationSpeech} Next question: ${QUIZ_QUESTIONS[stepIndex].label}`;
  ...
  const handleForceStartMic = () => {
    ...
    // Explicitly shut up the AI immediately
    if (window.speechSynthesis) {
      window.speechSynthesis.cancel();
    }
    isSpeakingRef.current = false;

    // Start the microphone tracking immediately for the CURRENT step state
    startSpeechRecognition();
  };
  ...
  const sendToBackend = async (spokenText) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/parse-voice`, {
        ...
        body: JSON.stringify({
          currentQuestionId: currentQuestion.idNum,
      ...
      // GUARD: If the AI backend indicates it failed to collect a clean value, re-prompt the current step!
      ...
      // Save extracted data properties to state
      setVoiceData(prev => ({ ...prev, [parsedResult.field]: parsedResult.value }));
      ...
      if (stepIndex < QUIZ_QUESTIONS.length - 1) {
        ...
        setTimeout(() => onComplete({ ...voiceData, [parsedResult.field]: parsedResult.value }), 3500);
      }
    } catch (err) {
      console.error("Network communication exception:", err);
      setIsLoading(false);
      speakText("Sorry, my system glitched out. Could you say that one more time?");
    }
  };

  return (
    <div className="w-full max-w-md bg-zinc-900 text-white rounded-3xl p-8 shadow-2xl flex flex-col items-center border border-zinc-800">
      ...
      <div className="h-24 flex items-center justify-center gap-1.5 mb-8">
        {[...Array(7)].map((_, i) => (
          <div 
            key={i} 
            className={`w-1.5 rounded-full bg-emerald-400 transition-all duration-300 ${isListening ? 'animate-pulse h-12' : isLoading ? 'animate-bounce h-8 bg-amber-400' : 'h-3 bg-zinc-700'}`}
      ...
      {transcript && (
        <div className="bg-zinc-800/60 rounded-2xl p-4 w-full text-center border border-zinc-700/40 mb-6">
...
{currentQuestion && (
  <div className="w-full mb-6 animate-fade-in">
    ...
    <div className="flex flex-wrap gap-1.5 justify-center">
      {/* 1. If the question has explicit option arrays (like preferences) */}
      ...
      {/* 2. Fallback placeholder examples (like for height/weight numbers) */}
      {!currentQuestion.options && currentQuestion.placeholder && (
        <div className="text-xs text-zinc-500 italic bg-zinc-950 px-4 py-2 rounded-xl border border-zinc-900/60 font-medium">
          Try saying: <span className="text-zinc-400 not-italic">"My {currentQuestion.id} is {currentQuestion.placeholder}"</span>
        </div>
      )}

      {/* 3. Global Skip Reminder if the step is optional */}
      ...
      <button
        ...
        disabled={isLoading}
        className={`w-full py-4 rounded-2xl font-bold transition-all shadow-lg text-sm tracking-wide ${isListening ? 'bg-zinc-800 text-emerald-400 border border-zinc-700' : isLoading ? 'bg-amber-500 text-black animate-pulse' : 'bg-emerald-500 hover:bg-emerald-400 text-black'}`}
      >
